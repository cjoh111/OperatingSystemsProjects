...
void main() {
   printf("ACO350: Advisement - Cole Johnson\n\n");
   
   // Initialize the random seed for the simulation
   init_random();

   // Initiaize the semaphores
   sem_init(&professorReady, 0, 0); //mutex lock
   sem_init(&studentDone,    0, 0); //mutex lock
   sem_init(&accessHallSeats,  0, 1); //mutex lock
   sem_init(&studentReady,   0, 0); //semaphore (num of students waiting)
   ...
   // Wait for all of the students to complete before exiting
   for(int i = 0; i < sim.num_students; i++) {
